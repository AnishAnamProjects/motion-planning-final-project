...
class MultiAgentDroneSimulation:
    def __init__(self, n_surveyors=3, n_workers=1, space_limit=10.0):
    ...
    def initialize_drones(self, start_positions=None):
        ...
        if start_positions is None:
            ...
            for i in range(self.n_drones):
                ...
                col = i % 2
                pos = np.array([
                    self.x_min + 1.0 + col * (self.space_limit - 2.0),
                    self.y_min + 1.0 + row * (self.space_limit - 2.0) / 2,
                    self.z_min + 2.0  # Start higher off the ground
                ])
    ...
    def control_loop(self, frame_idx):
        ...
        for i in range(self.n_drones):
            ...
            if dist_to_target < self.SPEED * 5 and dist_to_target > self.SPEED:
                # Add some randomness to break out of potential local minimum
            ...
            if dist_to_target < self.SPEED:
                ...
                self.SPEED = 0.3  # Increased from 0.1
...
if __name__ == "__main__":
    # Create simulation with 3 surveyors and 2 workers
    sim = MultiAgentDroneSimulation(n_surveyors=3, n_workers=2, space_limit=10.0)
