...
class MultiAgentPlanner:
    ...
    def get_next_waypoint(self, drone_id, drone_type, current_pos):
        ...
        if drone_type == 'surveyor':
            ...
            if self.phase == "exploration":
                ...
                print(f"\n=== ASSIGNING WORKERS TO TARGET ===")
                # Switch to execution phase
                self.phase = "execution"
                
                # Generate paths for all workers
                for i in range(self.n_workers):
                    # For testing purposes, provide position
                    print(f"Worker {i} current position: {current_pos}")
                    
                    # Get current position in grid coordinates
                    start_grid_pos = self.world_to_grid(current_pos)
                    path = self.find_path(start_grid_pos, self.target_grid_pos)
                    
                    if path:
                        self.worker_paths[i] = [self.grid_to_world(p) for p in path]
                        print(f"Worker {i} assigned path to target with {len(path)} waypoints")
                    else:
                        # If A* fails, create a direct path to target
                        direct_path = [current_pos, self.target_pos]
                        self.worker_paths[i] = direct_path
                        print(f"Worker {i} assigned direct path to target")
            ...
            if self.phase == "execution":
                ...
                if path and len(path) > 0:
                    ...
                    if dist < 0.5:  # Threshold for reaching waypoint
                        ...
                        if len(path) > 0:
                            print(f"Worker {drone_id} reached waypoint, {len(path)} remaining")
                            return path[0]
                        else:
                            # Reached target, hover
                            print(f"Worker {drone_id} has completed path to target")
                            return self.target_pos
                    return next_pos
                else:
                    # No path or empty path, just head toward target
                    if self.target_pos is not None:
                        print(f"Worker {drone_id} heading directly to target")
                        return self.target_pos
                    else:
                        return current_pos
    ...
    def find_path(self, start, goal):
        """Find a path using the shared map's pathfinding"""
        return self.shared_map.find_path(start, goal)
