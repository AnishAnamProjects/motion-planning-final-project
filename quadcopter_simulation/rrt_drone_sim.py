...
class RRTStarDroneSimulation(MultiAgentDroneSimulation):
    ...
    def __init__(self, filename = "", cloud_res=1.0, n_surveyors=3, n_workers=2, space_limit=10.0):
        """Initialize simulation with RRT* path planning"""
        # Initialize parent class
        super().__init__(filename, cloud_res, n_surveyors, n_workers, space_limit)
        ...
        self.max_simulation_frames = 5000  # Maximum frames before forced end
        self.frame_count = 0
    ...
    def initialize_drones(self, start_positions=None):
        ...
        if start_positions is None:
            # Create a more spread out distribution of start positions
            start_positions = []
            
            # Surveyors - put each in a different corner of the space at low height
            corners = [
                [1.0, 1.0, 2.0],  # Front left
                [self.space_limit/2 - 1.0, 1.0, 2.0],  # Front right
                [1.0, self.space_limit - 1.0, 2.0],  # Back left
                [self.space_limit/2 - 1.0, self.space_limit - 1.0, 2.0],  # Back right
            ]
            
            # Place surveyors at corners
            for i in range(self.n_surveyors):
                pos = corners[i % len(corners)]
                start_positions.append(np.array(pos))
            
            # Workers - place in the middle at higher altitude
            middle = np.array([self.space_limit/2, self.space_limit/2, self.space_limit/2])
            
            # Place workers around the middle
            for i in range(self.n_workers):
                angle = i * (2 * np.pi / self.n_workers)
                offset = np.array([np.cos(angle), np.sin(angle), 0]) * 2.0  # 2.0 units from center
                pos = middle + offset
    ...
    def control_loop(self, frame_idx):
        ...
        # Check if we need to add a target
        # if self.frame_count % 30 == 0:  # Check every 30 frames
        #     self.check_add_target()
        ...
        for i in range(self.n_drones):
            ...
            if i >= self.n_surveyors and self.drone_stuck_counters[i] >= self.stuck_threshold and self.planner.planner.target_pos is not None:
                ...
                if distance > self.SPEED * 10:  # If significantly far from target
                    # Move much closer to the target (60% of the way there)
                    new_pos = current_pos + direction * 0.6
                    ...
                    if not self.check_collision(new_pos):
                        ...
                        # If collision, try a different approach - move a bit to the side
                        ...
                        if np.linalg.norm(perp_direction) > 0:
                            ...
                            perp_pos = current_pos + perp_direction * 2.0  # Move 2 units perpendicular
                            
                            if not self.check_collision(perp_pos):
                                print(f"Worker {i - self.n_surveyors} was stuck too long, moving perpendicular to target direction")
                                self.drones[i].state[0:3] = perp_pos
                                current_pos = perp_pos
                                self.drone_stuck_counters[i] = 0  # Reset counter
    ...
    # def check_add_target(self):
    #     """Add a target to the environment if not found after a certain time"""
    #     # If target already found, return early
    #     if self.planner.planner.target_pos is not None:
    #         return
        
    #     # Check if we've gone too long without finding the target
    #     elapsed_time = time.time() - self.simulation_start_time
        
    #     # After 30 seconds, place a target (adjust timing as needed)
    #     if elapsed_time > 30.0:
    #         print("\n=== ADDING TARGET AT KNOWN LOCATION AFTER TIMEOUT ===")
            
    #         # Choose a location for the target - somewhere in the middle of known free space
    #         known_map = self.planner.planner.shared_map.grid
    #         free_mask = (known_map == 0)  # Free cells
            
    #         if np.any(free_mask):
    #             # Get indices of free cells
    #             free_indices = np.array(np.where(free_mask)).T
                
    #             # Pick a random free cell
    #             if len(free_indices) > 0:
    #                 # Bias toward middle z height
    #                 middle_height_indices = []
    #                 for idx in free_indices:
    #                     if known_map.shape[2] // 4 <= idx[2] <= 3 * known_map.shape[2] // 4:
    #                         middle_height_indices.append(idx)
                    
    #                 if middle_height_indices:
    #                     target_grid_pos = middle_height_indices[np.random.randint(len(middle_height_indices))]
    #                 else:
    #                     target_grid_pos = free_indices[np.random.randint(len(free_indices))]
                    
    #                 # Convert to world coordinates
    #                 target_pos = self.planner.grid_to_world(target_grid_pos)
                    
    #                 # Update the map and planner
    #                 self.planner.planner.target_pos = target_pos
    #                 self.planner.planner.target_grid_pos = target_grid_pos
                    
    #                 # Mark as target in the truth map
    #                 self.truth_map[target_grid_pos[0], target_grid_pos[1], target_grid_pos[2]] = 2
                    
    #                 # Also mark in the known map
    #                 self.planner.planner.shared_map.grid[target_grid_pos[0], target_grid_pos[1], target_grid_pos[2]] = 2
                    
    #                 # Set the target found time
    #                 if self.planner.planner.shared_map.target_found_time is None:
    #                     self.planner.planner.shared_map.target_found_time = elapsed_time
    #                     self.metrics["time_to_target"] = elapsed_time
                    
    #                 # Switch to planning phase
    #                 self.planner.planner.phase = "planning"
                    
    #                 print(f"Target added at {target_pos}, grid position {target_grid_pos}")
    ...
    #         # If we couldn't find a free cell, try again later
    #         return False
