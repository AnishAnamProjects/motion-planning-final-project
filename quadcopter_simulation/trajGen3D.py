...
def generate_trajectory(t, v, waypoints, coeff_x, coeff_y, coeff_z):
    ...
    # stay hover at the last waypoint position
        ...
        current_norm = LA.norm(current_heading)
        next_norm = LA.norm(next_heading)
        
        # Only update yaw if we have non-zero velocity
        if current_norm > 1e-6 and next_norm > 1e-6:
            dot_product = np.dot(current_heading, next_heading)
            # Clamp dot product to avoid numerical errors
            dot_product = np.clip(dot_product / (current_norm * next_norm), -1.0, 1.0)
            delta_psi = np.arccos(dot_product)
